...
`keycloak.ingress.hostName`, `rasa.rasa.ingress.hosts[0].host`). No host value
...
- The `&dns_hostname` YAML anchor and its `config.ingressHost` key were
  ...
  **`global.ingressHost`** once — the app, Keycloak and Rasa Pro model-service
  ...
  `app.ingress.hostName` (plus `keycloak.ingress.hostName` while Keycloak is
  enabled) and `rasa.rasa.ingress.hosts[0].host` (model service) —
