...
- **`charts/studio/`** - Rasa Studio (v2.4.0): multi-component app with backend, web-client, keycloak, event-ingestion
- **`charts/rasa/`** - Rasa Pro (v2.0.5): main Rasa Pro server, action-server, duckling, rasa-pro-services
...
The `charts/studio/` chart ships a `values.schema.json` that validates user-supplied values at `helm install`/`helm upgrade` time. Key patterns validated:
- `config.connectionType`: enum `["http", "https"]`
- `imagePullPolicy`: enum `["Always", "IfNotPresent", "Never"]`
- Secret references: object with required `secretName` + `secretKey` (see `$defs.secretRef`)
- Dual-type fields (string or secret ref): use `$defs.secretRefOrString`
- Component toggles (`enabled`): boolean

When adding new values to `charts/studio/`, update `values.schema.json` accordingly. `helm lint --strict` validates the schema against default values, so defaults must satisfy the schema.
