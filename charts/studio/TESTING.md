...
- **v1.30.13** (stable, recommended for CI/CD)
- **v1.31.9** (latest stable)

For local testing, we recommend using `kindest/node:v1.30.13` as it's the most stable version.
...
2. **Create a test cluster:**
   ```bash
   kind create cluster --name test-cluster --image kindest/node:v1.30.13
   ```
