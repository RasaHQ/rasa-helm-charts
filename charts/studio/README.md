...
![Version: 2.4.0-rc.0](https://img.shields.io/badge/Version-2.4.0--rc.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square)
...
- A Kafka broker (if `eventIngestion.enabled: true`)
...
```console
$ helm install my-release oci://europe-west3-docker.pkg.dev/rasa-releases/helm-charts/studio --version 2.4.0-rc.0
```
...
```console
$ helm install my-release rasa/studio --version 2.4.0-rc.0
```
...
```console
$ helm pull oci://europe-west3-docker.pkg.dev/rasa-releases/helm-charts/studio --version 2.4.0-rc.0
```

### From GitHub Helm Repository:

```console
$ helm pull rasa/studio --version 2.4.0-rc.0
```
...
**If a migration fails**, the job is not automatically cleaned up (only successful jobs are deleted). Before retrying `helm upgrade`, manually delete the failed job:

```console
$ kubectl delete job <release-name>-studio-database-migration
```

Then re-run `helm upgrade` after investigating and resolving the root cause.

The migration job waits for PostgreSQL to be ready when `backend.migration.waitForIt: true` (default). Disable this only if your database is guaranteed reachable at deploy time.
