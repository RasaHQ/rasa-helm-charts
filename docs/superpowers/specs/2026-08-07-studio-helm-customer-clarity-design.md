...
**Date:** 2026-08-07  
**Status:** Locked (customer-clarity items 1, 2, 7, 8)  
...
1. SPA overrides live under a top-level `webClient:` key that looks like a separate component (it is not).
2. Dead / misleading `MS_API_URL` guidance on the **app** container env — Studio API process does not read `MS_API_URL`; only the SPA uses `window.MS_API_URL`.
...
- Make SPA config obviously belong to the unified `app` component.
...
### 1. Nest SPA config: `webClient` → `app.webClient`
...
Rationale: there is no separate web-client workload; SPA `config.js` is mounted on the app pod. Nesting under `app` matches the unified process and stops customers hunting for a removed component.

**Upgrade:** Hard-break — no Helm alias from top-level `webClient:` → `app.webClient:`. Document values translation in README upgrade notes (alongside existing `backend` → `app` migration).

### 2. Drop dead app-container `MS_API_URL`; keep SPA `MS_API_URL`
...
| Keep | SPA override key name `MS_API_URL` under `app.webClient.environmentVariables` — still writes `window.MS_API_URL` in the SPA ConfigMap |
| Keep | Default `window.MS_API_URL` derivation via `studio.modelServiceBaseUrl` when the SPA override is unset |
...
- `connectionType` is the scheme (`http` / `https`) for **externally derived** URLs (ingress-based `API_URL`, `WEB_CLIENT_URL`, SPA `API_ENDPOINT`, model-service public URLs, `CORS_ORIGINS`, etc.).
...
| File | Change |
| --- | --- |
| `charts/studio/values.yaml` | Move `webClient:` under `app:`; fix `connectionType` comment; remove app `MS_API_URL` docs; clarify eventIngestion applicability comments |
...
| `charts/studio/templates/studio/app/deployment.yaml` | Drop `MS_API_URL` default dict merge; range `app.environmentVariables` only |
| `charts/studio/README.md.gotmpl` | Nest examples; remove app `MS_API_URL` guidance; SPA override under `app.webClient`; eventIngestion applicability; upgrade note `webClient` → `app.webClient` |
| `charts/studio/NOTES.txt` | Only if it mentions `webClient` / app `MS_API_URL` (today it does not for SPA); touch only if needed for upgrade hints |
...
- Spot-check README/values comments for connectionType and eventIngestion mode applicability
...
- Implementers treat nesting + MS_API_URL cleanup + comment fixes as one small clarity PR (or plan tasks), without reopening scope.
- Customer Helm values show SPA config under `app.webClient` only.
