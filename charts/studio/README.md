...
- The `&dns_hostname` YAML anchor was removed. For a single-host install set
  **`global.ingressHost`** once — the app, Keycloak and Rasa Pro model-service
  ...
  `config.ingressHost` (app/Keycloak) plus `rasa.rasa.ingress.hosts[0].host`
  (model service) — `global.ingressHost` overrides the per-host model-service
  value when set, so the two are mutually exclusive by design.
...
| global.ingressHost | string | global.ingressHost is the single-host knob: set it once and the Studio app ingress, Keycloak ingress, Rasa Pro model-service ingress (Helm propagates globals to the subchart natively) and every derived URL (window.MS_API_URL, RASA_MODEL_SERVER_BASE_URL, WEB_CLIENT_URL, CORS) resolve from it. WARNING: when set it silently overrides per-component hosts, including rasa.rasa.ingress.hosts[0].host — leave it UNSET for split-host installs and use config.ingressHost + rasa.rasa.ingress.* instead. | `nil` |
