...
| backend.environmentVariables.SEED_USER_PASSWORD | object | backend.environmentVariables.SEED_USER_PASSWORD is the password for the default users seeded on backend startup (admin@studio.local and member@studio.local). Required in production — the backend exits if unset. Must be at least 8 characters. Stored in a Kubernetes secret. Required. | `{"secret":{"key":"SEED_USER_PASSWORD","name":"studio-secrets"}}` |
